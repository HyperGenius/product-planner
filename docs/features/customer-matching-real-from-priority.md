...
- `resolve_or_create_customer` に `real_from_email: str | None = None` を追加し、優先順位を
  ...
  4. それでも一致しない場合の下書き作成では、本文から抽出できた候補（`candidates`）が
     あればそれを優先し、候補が1件も無い場合のみ `real_from_email` を作成用メールアドレス
     として使う。候補を優先するのは、Issue #298 の「直接転送」（社内担当者が転送機能を
     介さずに送信し直したためヘッダー行が本文化されないケース）では実際の Gmail `From`
     ヘッダーが社内担当者のアドレスになり、本文の署名欄の方が信頼できるため
     （レビューで指摘された、本文候補があるにもかかわらず実Fromヘッダー＝社内担当者の
     アドレスで下書きが作られてしまう不具合の修正）
