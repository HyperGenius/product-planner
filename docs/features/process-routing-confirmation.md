...
## 工程管理モーダルの確定状態表示の重複解消（Issue #334）

工程名列に「確定済み」バッジ（緑色）を表示していたが、同テーブルの「確定」チェック列（緑背景＋チェックマーク）と同じ `is_confirmed` を二重に表現しており、バッジ分だけ工程名列の幅が伸びて行のレイアウトが崩れる問題があった。

### Frontend の変更

| ファイル | 変更内容 |
|---|---|
| `components/product-routings-dialog.tsx` | 工程名セルの「確定済み」バッジ表示を削除。確定状態は「確定」チェック列のみで表現するよう統一。未使用になった `CheckCircle2` の import を削除。「未保存」バッジ（`id < 0` の新規行）は維持 |
