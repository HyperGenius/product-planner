...
import { getProductDisplayParts } from "@/lib/order-utils"
import { IN_PRODUCTION_STATUSES, toJstDateIso } from "@/lib/floor-dashboard-utils"
...
export function ShipmentScheduleList({
  ...
  const groups = useMemo(
    () => groupSchedulesByShipmentDate(schedules, orders, products),
    [schedules, orders, products],
  )
  ...
  return (
    <section className="rounded-lg border border-border bg-card p-6 shadow-sm">
      ...
      {isLoading ? (
        ...
        <p className="text-muted-foreground">出荷予定はありません</p>
      ) : (
